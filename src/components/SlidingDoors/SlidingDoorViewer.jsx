...
import { setSlidingDoorHighlighted } from "../../slices/shelfDetailSlice";

const SlidingDoorViewer = ({ doorKey, type, position, width, section }) => {
  const dispatch = useDispatch();
  const color = useSelector((state) => state.shelfDetail.racks.execution.color);
  const selectedSection = useSelector(
    (state) => state.shelfDetail.racks.selectedSection
  );
  const isRevDoorSelected = useSelector(
  ...
  return (
    <button
      className={`Deuren_Deuren__YKK11 Section_deuren__mXI7h ${
        color == "black" ? "door_black" : ""
      } ${type}_${width} Deuren_clickable__7t+am
         ${
           isRevDoorSelected?.id == doorKey && section == selectedSection
             ? "isHighlighted"
             : ""
         } ${hideDoor ? "opacity-20 !pointer-events-none" : ""}`}
...
export default SlidingDoorViewer;
