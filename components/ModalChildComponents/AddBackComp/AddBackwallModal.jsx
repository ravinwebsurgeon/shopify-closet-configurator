    ...
    <>
      ...
      <div className="data-container-div flex flex-col">
        <p className="text-base text-[#939393]">
          Selecteer de gewenste hoogte van de achterwand. Het is na het
          selecteren mogelijk de achterwand naar de gewenste hoogte te
          verplaatsen.
        </p>
        <p className="text-base text-[#939393]">
          Bij het selecteren van een kleinere hoogte achterwand is het mogelijk
          meerdere wanden te stapelen.
        </p>
        <div className="radioBtn-container flex flex-wrap gap-4 mt-5">
          {heightOptions.map((height) => (
            <button
              className={`text-base border px-2 py-1 rounded-sm w-1/4
                    ${
                    ...
                    ${
                      height > currHeight
                        ? "cursor-not-allowed bg-gray-200 opacity-50 pointer-events-none"
                        : "hover:bg-gray-100"
                    }
                    `}
              disabled={height > currHeight}
              key={height}
              onClick={(e) => handleSelectedHeight(e, height)}
            >
              <span>{height} cm </span>
            </button>
          ))}
...
export default AddBackwallModal;
